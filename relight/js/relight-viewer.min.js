...
`;if(t.normals){if(t.normals==2)str+="uniform vec3 light;\n";str+=`
...
`;return str};Relight.prototype.fragTail=function(){var str="";switch(this.normals){case 1:str="\tcolor = (normalize(T * color) + 1.0)/2.0;\n";break;case 2:str=`	color = normalize(T * color);
	float c = dot(light, color);
	color = vec3(c, c, c);
`;break}str+=`	gl_FragColor = vec4(color, opacity);
...
`}src+=t.fragTail();return src};Relight.prototype.myccFrag=function(){var t=this;var src=t.headFrag();src+=`
vec3 toRgb(vec3 ycc) {
 	vec3 rgb;
...
`}src+=t.fragTail();return src};Relight.prototype.rgbFrag=function(){var t=this;var src=t.headFrag()+`
void main(void) {
	vec3 color = vec3(0);
	for(int j = 0; j < nj; j++) {
		vec4 c = texture2D(planes[j], v_texcoord);
...
`;src+=t.fragTail();return src};Relight.prototype.yccFrag=function(){var t=this;var src=t.headFrag()+`
vec3 toYcc(vec4 rgb) {
	vec3 c;
...
`;src+=t.fragTail()};Relight.prototype.lrgbFrag=function(){var t=this;var src=t.headFrag();if(!t.normals)src+=`
void main(void) {
...
`;src+=t.fragTail();return src};Relight.prototype.imgFrag=function(){var src=`#ifdef GL_ES
precision highp float;
#endif
...
}`;return src};Relight.prototype.demFrag=function(){var src=`#ifdef GL_ES
precision highp float;
#endif
...
}`;var frag;switch(t.type){case"img":frag=t.imgFrag();break;case"dem":frag=t.demFrag();break;default:switch(t.colorspace){case"mrgb":frag=t.mrgbFrag();break;case"mycc":frag=t.myccFrag();break;case"ycc":frag=t.yccFrag();break;case"rgb":frag=t.rgbFrag();break;case"lrgb":frag=t.lrgbFrag();break}}t.fragCode=frag};
